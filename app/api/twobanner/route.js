...
import SingleBannerTwo from "@/models/twobanner";
import fs from "fs";
import path from "path";
import sharp from "sharp";

// ✅ Save File Function with 640x200 dimension validation
async function saveFile(file) {
  try {
    ...
    // ✅ Enforce exact 640x200 dimensions
    if (metadata.width !== 640 || metadata.height !== 200) {
      throw new Error(
        `Image must be exactly 640x200 pixels. Your image is ${metadata.width}x${metadata.height} pixels.`
      );
    }

    const uploadDir = path.join(process.cwd(), "public", "uploads", "twobanner");
    ...
    return "/uploads/twobanner/" + filename;
  } catch (err) {
