...
useEffect(() => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch offers
        const offersResponse = await fetch("/api/deals/get");
  ...
  const handleDelete = async (offerId) => {
    try {
      const response = await fetch("/api/deals/delete", {
        method: "DELETE",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch("/api/deals/post", {
        method: "POST",
        headers: {
...
const filteredOffers = offers.filter((offer) => {
  ...
  const matchesSearch = offer.fest_offer_status
    .toLowerCase()
              ...
              {currentItems.length > 0 ? (
                currentItems.map((offer) => (
                  <tr key={offer._id} className="text-center border-b">
                     <td className="p-2 font-bold">{offer.notes}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-3xl mx-4 max-h-[90vh] flex flex-col">
            ...
            <div className="flex justify-between items-center border-b-2 border-gray-300 px-6 py-4">
              <h2 className="text-xl font-semibold text-gray-900">Create Deals Products</h2>
              <button
