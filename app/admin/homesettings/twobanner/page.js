...
export default function TwoBannerPage() {
  ...
  const fetchBanners = async () => {
    try {
      const res = await fetch("/api/twobanner");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.message.includes("640x200")) {
  ...
  return (
    <div className="container mx-auto">
      <div className="flex justify-between items-center mb-5 mt-5">
        <h2 className="text-2xl font-bold">Two Banner Manager</h2>
        <Link
            ...
            <div className="border p-4 rounded-lg space-y-3">
              ...
              <div>
                <p className="text-gray-500 text-sm mb-2">
                  Banner size: <span className="font-semibold">640 × 200 px</span>
                </p>
                <input
                  type="file"
                  onChange={(e) =>
